...
class User
{
    ...
    function __construct($id, $nickname, $email, $password)
    {
        $this->id = $id;
    ...
    static function all()
    {
        ...
        foreach ($req->fetchAll() as $item) $list[] = new User($item['id'], $item['nickname'], $item['email'], $item['password']);
    ...
    static function find($id)
    {
        ...
        $item = $req->fetch();
        if (isset($item['id'])) {
            return new User($item['id'], $item['nickname'], $item['email'], $item['password']);
        }
        return null;
    }
    ...
    static function login($email, $pwd){
        ...
        $record = $req->fetch();
        return isset($record['id']) ? new User($record['id'], $record['nickname'], $record['email'], $record['password']) : null;
    }
